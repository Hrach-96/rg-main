...
// Get Sku code
$query_result = getwayConnect::$db->query("SELECT * FROM delivery_deliverer");
$total_result = [];
...
if(isset($_POST)){
    if($_POST['action'] == "change_action"){
        $id = $_POST['id'];
        $val = $_POST['val'];
        getwayConnect::getwayData("UPDATE delivery_deliverer SET active = '" . $val . "' where id = '" . $id . "'");
    }
}
if(isset($_POST['action'])){
    if($_POST['action'] == "change_delivery_price_action"){
        $id = $_POST['id'];
        $val = $_POST['val'];
        getwayConnect::getwayData("UPDATE delivery_deliverer SET delivery_price = '" . $val . "' where id = '" . $id . "'");
    }
}
?>
...
<div class='row' style='margin-top:88px'>
    <div class='col-md-12'>
        <a style='margin-left:20px;' href="deliverer_info.php" class='btn btn-primary'>Add Deliverer</a>
    </div>
</div>
    ...
    <table id="data_table" class="table table-bordered table-hover">
        ...
        foreach ($total_result as $key => $value) {
            ...
            <tr >
                ...
                <td>
                    <label class="switch">
                      <input type="checkbox" <?= ($value['delivery_price'] == 1)? 'checked' : '' ?> data-id="<?=$value['id']?>" class='SwitchInputDeliveryPrice'>
                      <span class="slider round"></span>
                    </label>
                </td>
                <td>
                    <label class="switch">
                      <input type="checkbox" <?= ($value['active'] == 1)? 'checked' : '' ?> data-id="<?=$value['id']?>" class='SwitchInput'>
                      <span class="slider round"></span>
                    </label>
                </td>
                <td>
                ...
                <td>
                    <a target='_blank' href="deliverer_info.php?deliverer_id=<?php echo $value['id'] ?>" class="edit_item" title="Փոփոխել">
...
<script type="text/javascript">
    $(document).ready(function(){
        ...
        $(document).on("change",".SwitchInputDeliveryPrice",function(){
            var id = $(this).attr('data-id');
            var val;
            if($(this).prop('checked')){
                val = 1
            ...
            $.ajax({
                type:"post",
                data:{action:"change_delivery_price_action",id:id,val:val},
                success:function(res){
                }
            })
        })
