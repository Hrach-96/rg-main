...
// Get Sku code
$query_result = getwayConnect::$db->query("SELECT * FROM translate_of_names");
$total_result = [];
foreach ($query_result as $row) {
    $total_result[] = $row;
}
if(isset($_POST['action'])){
    if($_POST['action'] == "change_gender_of_names"){
        $id = $_POST['id'];
        $val = $_POST['val'];
        getwayConnect::getwayData("UPDATE translate_of_names SET gender = '" . $val . "' where id = '" . $id . "'");
    }
    if($_POST['action'] == "delete_row"){
        $id = $_POST['row_id'];
        getwayConnect::getwayData("delete from translate_of_names where id = '" . $id . "'");
        return true;
    }
}
?>
...
<div class='row' style='margin-top:88px'>
    <div class='col-md-12'>
        <a style='margin-left:20px;' href="translated_name_info.php" class='btn btn-primary'>Add Info</a>
    </div>
</div>
    ...
    <table id="data_table" class="table table-bordered table-hover">
        ...
        foreach ($total_result as $key => $value) {
            ...
            <tr >
                ...
                <td>
                    <label class="switch">
                      <input type="checkbox" <?= ($value['gender'] == 1)? 'checked' : '' ?> data-id="<?=$value['id']?>" class='switchInputGenderOfNames'>
                      <span class="slider round"></span>
                    </label>
                </td>
                <td>
                    <a target='_blank' href="translated_name_info.php?info_id=<?php echo $value['id'] ?>" class="edit_item" title="Փոփոխել">
                        <i class="glyphicon glyphicon-edit"></i>
                    </a>
                    <i data-row-id="<?php echo $value['id'] ?>" title="Ջնջել" class="glyphicon glyphicon-remove remove_item hoverCursorPointer text-primary"></i>
                </td>
            </tr>
        <?php } ?>
        </tbody>
    ...
    $(document).ready(function(){
        ...
        $(document).on("click",".remove_item",function(){
            if (window.confirm("Are you sure?")) {
                ...
                $.ajax({
                    type:"post",
                    data:{action:"delete_row",row_id:row_id},
        ...
        $(document).on("change",".switchInputGenderOfNames",function(){
            ...
            $.ajax({
                type:"post",
                data:{action:"change_gender_of_names",id:id,val:val},
                success:function(res){
                }
            })
        })
