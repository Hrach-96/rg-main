	...
	<body>
		...
		<script>
			...
			function filter(el,onfilter) {
				...
				$.get("<?=$rootF?>/data.php?cmd=data&page="+data_type+send_data+"&paginator="+fromP+":"+toP, function (get_data){
					...
					if (countP > 0) {
						for(var i = 0;i < tableData.length;i++)
						{
							...
							htmlData += "<tr data-id='"+d.id+"' "+showA+">";
							...
							htmlData += "<td style=\"min-width:50px;\" nowrap>N-"+d.id+"<br/><br><img src=\"<?=$rootF?>/template/icons/status/"+d.delivery_status+".png\" title=\""+statusTitle[d.delivery_status]+"\"><br/><div style=\"position:relative;max-width:90px;\"><img width=\"90px\" style='border: 1px solid "+car_color+"' src=\"<?=$rootF?>/template/icons/deliver/"+d.delivery_type+".png\">"+delvrr+"</div></td>";
							...
							window.bacik = '';
							if (d.greetings_card){
								window.bacik = "<div class=\"article\" ><button data-order-id='" + d.id + "' class=\"readMoreButtonAjaxGreetingCard\"><img width=\"15px\" height=\"15px\" src=\"ico/greeting.png\"/> "+d.greetings_card+"</button><div class='display-none forAjaxGreetingCardValue_" + d.id + "'><span></span></div></div>";
							}
							window.nflorist = '';
							if (d.notes_for_florist){
								window.nflorist = "<div class=\"article\" ><button data-order-id='" + d.id + "' class=\"readMoreButtonAjaxFloristNote\"><img width=\"15px\" height=\"15px\" src=\"ico/notes.png\"/> "+d.notes_for_florist+"</button><div class='display-none forAjaxFloristNoteValue_" + d.id + "'><span></span></div></div>";
							}
							...
							if(d.delivery_price > 0){
								...
								<?php if (isset($userData[0]) && strtolower($userData[0]["username"]) == "hovik" || strtolower($userData[0]["username"]) == "norik") { ?>
									actiondata += "<br>Ստացող:"+delivery_reciever[d.who_received];
								<?php }else { ?>
									actiondata += "<br>Ստացող:"+delivery_reciever[d.who_received]+" AMD:"+drive_prices[d.delivery_price];
								<?php } ?>
							}
							
							
							htmlData += "<td style=\"min-width:200px;\">"+d.product+actiondata;
							...
							if(d.delivery_status == "6"){
								...
								actiondata += "<select style=\"max-width:90px; position: absolute; \"  id=\"actualReceiver-"+d.id+"\" onchange=\"enable_price("+d.id+")\">";
								...
								if((d.delivery_price == 0 && <?=(max(page::filterLevel(4,$levelArray)) >= 49 ) ? "true" : "false";?>) || (d.delivery_price == 0 ) || '<?=(in_array(99,explode(',',$level[0]["user_level"])))? 99 : 0;?>' > 98){
									...
									if(delivery_first_time != ''){
										<?php
											if($driverInfo[0]['delivery_price'] == 0){
												?>
													if(minDiff < 20){
												...
												?>
													if(minDiff > 20){
														actiondata += " <select style=\"max-width:90px;\"  id=\"drivePrice-"+d.id+"\">";
														actiondata += "<option value=\"0\">Price</option>";
														for(var key = 0 ; key < drivePricesArrayNew.length ; key++){
															if(drivePricesArrayNew[key]['id'] != 4){
																actiondata+= "<option value='" + drivePricesArrayNew[key]['id'] + "'>" + drivePricesArrayNew[key]['name'] + "</option>";
															}
														}
														actiondata += "</select>";
													}
							...
							htmlData += "<input type='checkbox' class='wrongAddress' id='wrongAddress" + d.id + "'";
				...
				//end
				setTimeout(() => {
					$(".total_delivery_price").html("<b>Ցուցակի ընդհանուր առաքման գումար` " + total_delivery_price + "</b>");
					addWiki();
				}, 1000);
			},2000);
				return false;
			...
			function onit(id){
				...
				alert('Շնորհակալություն , Դուք հաստատեցիք որ այս պատվերի առաքումը կարողեք կատարել նշված ժամանակահատվածում։');
			}
			function onroad(id){
			...
			function send_drivedata(id){
				...
				if($('#receiver_mood').val() && $('#receiver_mood').val() != 0){
					...
					if($('#drivePrice-'+id).val() != 0 && $('#actualReceiver-'+id).val() != 0 ){
						alert('Շնորհակալություն ձեր առաքման ծանուցւոմը գրանցվեց համակարգում')
						...
						if($('#drivePrice-'+id).length){
							request_call('&id='+id+'&who_received='+$('#actualReceiver-'+id).val()+'&delivery_price='+$('#drivePrice-'+id).val()+"&delivery_status=3&receiver_mood="+mood);
						}
						else{
							request_call('&id='+id+'&who_received='+$('#actualReceiver-'+id).val()+"&delivery_status=3&receiver_mood="+mood);
						}
						...
						alert('Nsheq bolor tvyalner@'); 
					}
				} else {
					alert('Nsheq tramadrutyuny!');
				}
			}
