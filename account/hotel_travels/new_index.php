...
if(!isset($_GET['end_date']) && !isset($_GET['start_date'])){
    // by default today
    $sql.=" check_in >= '" . date("Y-m-d") . "'";
    $sql.=" and check_out <= '" . date("Y-m-d") . " 23:59:00'";
}
if(isset($_GET['start_date'])){
    $sql.=" check_in >= '" . $_GET['start_date'] . "'";
}
if(isset($_GET['end_date'])){
    $sql.=" and check_in <= '" . $_GET['end_date'] . " 23:59:00'";
}
					...
					<select name="vpartneID" id="vpartneID">
                        ...
                        <div class="input-group" style="min-width:790px;display:none" data-rel-hotel="true">
                         ...
                         <td><input type="text" placeholder="checkout" name="check_out"
                            ...
                            <span class="input-group-addon" style='text-align: left'>
                                <span>
                                    <input type='checkbox' class='checkInPerson'> Check In person:
                                    <br>
                            ...
                            <span class="input-group-addon" style='text-align: left'>
                                <span>
                                    <input type='checkbox' class='checkOutPerson'> Check Out person:
                                    <br>
...
<script>
   new DataTable('#mainTable');
    ...
    function saveButtonCheckInputs() {
        var inputArrays = Array('vprice','vrevenue','vpartial_pay');
        var sendRequest = true;
        for(var i = 0 ; i < inputArrays.length;i++){
            if($("#"+inputArrays[i]).val() == ''){
                sendRequest = false;
