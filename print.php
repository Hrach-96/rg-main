...
// if ($file = fopen("couriers_list_2020.txt", "r")) {
//     while(!feof($file)) {
//         $line = fgets($file);
//         $info = explode('|', $line);
//         if(count($info) > 1){
...
//     fclose($file);
// }
$delivery_deliverers = getwayConnect::getwayData("SELECT * FROM delivery_deliverer WHERE active = 1");
foreach($delivery_deliverers as $key=>$delivery_deliverer){
    $deliverer_names[] = $delivery_deliverer['full_name'];
    $deliverer_informations[] = $delivery_deliverer['phone']; 
}
    ...
    if(isset($_POST['order_id']) && $_POST['order_id'] != "0" && isset($_POST['old_status']) && $_POST['old_status'] != "0"){
        ...
        if($status != "3"){
            if($status == "12"){
                ...
                $html_for_notes_for_pending_log = 'Ստատուս: <span style="color:blue">Պատրաստ </span> <span style="color:red">-></span> <span style="color:#4F6228"><b> Ճանապարհին';
                getwayConnect::getwaySend("INSERT INTO log_".$table_count  . " (order_id,description,operator_id,date,current_status_id) VALUES ('{$order_id}','{$html_for_notes_for_pending_log}','{$userData[0]['id']}','" . date("Y-m-d H:i:s") ."','12')");
            }
        ...
        .delivery_driver_information{
            ...
            <script>
                ...
                function saveName (driveid ,stage ) {
                   $(".save_" + driveid + "_" + stage).css({"color":"red"});
...
<form action="" method="POST">
	...
	<img src="https://new.regard-group.ru/account/flower_orders/ico/chvert.png" style="vertical-alig:middle;width:70px;align:left;"> Առաքման Ուղերթներ   
    ...
    <select name="selectedDriver">
      <?php
        ...
        foreach ($driverList as $value) {
            $simple_name = $value['name'];

            $value["name"] = getConstant($value["full_name"]);
            
            
               
            
            if (isset($_POST["selectedDriver"]) && $_POST["selectedDriver"] > 0 && $_POST["selectedDriver"] == $value["id"] ) {
                 echo "<option value=\"" . $value["id"] . "\" selected>" .$simple_name . " " . $value['full_name'] . "</option>";
            } else {
                 echo "<option value=\"" . $value["id"] . "\">" .$simple_name . " " . $value['full_name'] . "</option>";
            }
        }
      ?>
                ...
                $backgroundTr = 'style="background-color: #e8e8e8"';
                        ...
                        foreach ($arrayStages as $valueStages) {
							...
							if(count($deliverer_names)){
								...
								$selected_driver_name = getDriverName ($valueStages , $id, $datePost);
								$deliverer_phone_get = $deliverer_informations[array_search($selected_driver_name, $deliverer_names)];
								foreach($deliverer_names as $deliverer_key => $deliverer_name){
									$stagetableFirst .= '<option value="'.$deliverer_name.'"';
									//echo 2;
									if($selected_driver_name == $deliverer_name){
							...
							$stagetableFirst .= ' <input type="text" id="drivename_'.$id.'_'. $valueStages.'"  value="' .  getDriverName ($valueStages , $id, $datePost)  . '"  '. $editdatte . '> '
									. ' <button class="save_'.$id.'_'. $valueStages.'" onclick="saveName('. $id  . ', ' .$valueStages .  ')">SAVE</button> ';
                            ...
                            foreach ($query_data_result_step as $rowData) {
                                  ...
                                  if (max(page::filterLevel(3, $levelArray)) < 33) {
                                    ...
                                    $statusImage = '<img src="/template/icons/status/' . $rowData["delivery_status"] . '.png" data-order="'.$rowData["id"].'" data-status="'.$rowData["delivery_status"].'" style="width : 20px" />   &nbsp;';
                                  }
                                  ...
                                  $stagetable .= '<tr ' .  $backgroundTr . '>
                                        ...
                                        <td style="border-bottom: 1px solid black; font-size:18px;">';
                                        if(isset($rowData['strname']) && $rowData['strname'] != ''){
                                            $stagetable .= $rowData["strname"] . ", ";
                                        }
                                        $stagetable .= $rowData["receiver_address"]. " / ";
                                        ...
                                        $stagetable .= ' || '. $rowData["subname"].' <button data-order-id="' . $rowData['id'] . '" class="readMoreButtonAjaxGreetingCard">Բացիկ </button><div class="display-none forAjaxGreetingCardValue_' . $rowData['id'] . '" ></div> </td>
                                        <td style="border-bottom: 1px solid black; font-size:18px;"><a target="_blank" href="account/flower_orders/print.php?orderId=' . $rowData['id'] . '">' . $qty_var . '</a></td>
                                    </tr>';
                            ...
                            foreach ($query_data_result_time as $rowData) {
                                  ...
                                  if ( isset($rowData["delivery_time_manual"] ) && strlen($rowData["delivery_time_manual"] ) > 1) {
                                     $manualTime =   " (" . $rowData["delivery_time_manual"] . ' - ' . $rowData['travel_time_end'] . ") "  ;
                                  }
                                  ...
                                  $stagetable .= '<tr ' .  $backgroundTr . '>
                                        ...
                                        $notes_for_florist = '';
                                        if(!empty($rowData['notes_for_florist'])){
                                            $notes_for_florist = ' <br><span style="font-size:13px">(' .  $rowData['notes_for_florist'] . ')</span>';
                                        }
                                        $qty_var = $qty;
                                        if($qty != 1){
                                            $qty_var = '[]';
                                        }
                                        $stagetable .= ' || '. $rowData["subname"] . $notes_for_florist .' <button data-order-id="' . $rowData['id'] . '" class="readMoreButtonAjaxGreetingCard">Բացիկ </button><div class="display-none forAjaxGreetingCardValue_' . $rowData['id'] . '" ></div> </td>
                                        <td style="border-bottom: 1px solid black; font-size:18px;"><a target="_blank" href="account/flower_orders/print.php?orderId=' . $rowData['id'] . '">' . $qty_var . '</a></td>
                                    </tr>';
							...
							$stagetable .= '<p class="delivery_driver_information">'.$deliverer_phone_get.'</p>';
...
function  getDriverName ($stageid , $driveid, $date = null) {
     ...
     $query_check = "SELECT * FROM page_print WHERE DRIVERID = $driveid AND STAGE = $stageid";
                ...
                $.ajax({
                    ...
                    success: function(resp){
                        if(resp != '[]'){
                            ...
                            console.log(resp);
                            $(".forAjaxGreetingCardValue_"+order_id ).html("<br>" + resp[0].value + "<br>")
                        }
                    }
                })
            }
