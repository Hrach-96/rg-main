...
class page
{
    public static function buildMenu($userLevel)
    {
        ...
        if ($data != false && !empty($data)) {
            ...
            if (in_array(15, $ul)) {
                $menu .= "<li><a href=\"/print.php\">PRINT</a></li>";
            }
    ...
    public static function getDataByfilter($userLevel, $encodedData, $pageLevel, $limitQ = 50)
    {
                        ...
                        if ($table == $nextTable) {
                            ...
                            if ($filter) {
                                ...
                                if ($actions["by"] == "range") {
                                    ...
                                    //     $val = explode(' ',$encodedData[$i]["value"])[1];
                                    //     $tempQuery.= " `notes` like '%{$val}%' or `notes_for_florist` like '%{$val}%' or `notes_for_pending` like '%{$val}%' or `greetings_card` like '%{$val}%'";
                                    // }
                                    ...
                                    $tempQuery = "(" . rtrim($tempQuery, "OR ") . ") AND ";
                                    $queryArray[] = $tempQuery;
        ...
        if (!empty($queryArray) || isset($orderQ) || isset($prod_type_filter) || isset($prod_user_filter)) {
            ...
            if($table == "rg_orders"){
                $sub_query = "SELECT `{$table}`.*, `user`.`username` as `flourist`, `delivery_time`.`name` as `delivery_time_range`, `organisations`.`name_am` as `organisation_name`
...
//                    $condition .= " OR (DHB.check_in >= '$date_from' AND DHB.check_in <= '$date_to')) ";
            ...
            $qCount = getwayConnect::getwayCount($count_query);
